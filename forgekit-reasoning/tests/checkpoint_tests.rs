...
fn test_create_in_memory_storage() {
    ...
    assert!(
        storage.is_ok(),
        "Should be able to create in-memory storage"
    );
}
...
fn test_checkpoint_session_id() {
    ...
    assert_eq!(
        ...
        "Checkpoint should belong to correct session"
    );
}
...
fn test_checkpoint_state_has_env() {
    ...
    assert!(
        state.working_dir.is_some(),
        "State should have working directory"
    );

    // Verify environment variables were captured
    assert!(
        !state.env_vars.is_empty(),
        "State should have environment variables"
    );
}
...
fn test_get_checkpoint_by_id() {
    ...
    assert_eq!(
        checkpoint.message, "Retrievable checkpoint",
...
fn test_list_checkpoints_by_session() {
    ...
    assert_eq!(
        checkpoints.len(),
        3,
        "Should list all checkpoints in session"
    );
}
...
fn test_list_checkpoints_by_tag() {
    ...
    let cp1 = manager
        .checkpoint_with_tags(
            "Important",
            vec!["critical".to_string(), "release".to_string()],
        )
        .unwrap();
    let _cp2 = manager
        .checkpoint_with_tags("Also important", vec!["critical".to_string()])
        .unwrap();
...
fn test_create_file_based_storage() {
    ...
    assert!(
        storage.is_ok(),
        "Should be able to create file-based storage"
    );
}
...
fn test_export_checkpoints() {
    ...
    assert!(
        json.contains("Export test 1"),
        "Export should contain first checkpoint"
    );
    assert!(
        json.contains("Export test 2"),
        "Export should contain second checkpoint"
    );
}
...
fn test_checkpoint_compaction() {
    ...
    assert_eq!(
        checkpoints.len(),
        5,
        "Should have 5 checkpoints after compaction"
    );
    ...
    assert!(
        seqs.iter().all(|&s| s >= 5),
        "Should keep newest checkpoints"
    );
}
...
fn test_compaction_preserves_tags() {
    ...
    manager
        .checkpoint_with_tags("Important 1", vec!["preserve".to_string()])
        .unwrap();
    manager.checkpoint("Normal 1").unwrap();
    manager
        .checkpoint_with_tags("Important 2", vec!["preserve".to_string()])
        .unwrap();
    ...
    let checkpoints = manager.list().unwrap();
    let tagged_count = checkpoints
        .iter()
        .filter(|cp| cp.tags.contains(&"preserve".to_string()))
        .count();
...
fn test_delete_checkpoint() {
    ...
    // Verify cp2 is gone
    assert!(
        manager.get(&cp2).unwrap().is_none(),
        "Deleted checkpoint should be gone"
    );
}
