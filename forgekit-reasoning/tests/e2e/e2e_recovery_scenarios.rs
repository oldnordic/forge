...
fn e2e_recovery_from_corruption() {
    ...
    assert!(
        report.invalid == 0,
        "No checkpoints should be invalid in normal operation"
    );
}
...
fn e2e_tagged_compaction_recovery() {
    ...
    let result = service
        .execute(CheckpointCommand::Compact {
            session_id: session,
            keep_recent: 3,
        })
        .unwrap();
...
fn e2e_full_system_stress() {
    ...
    let sessions: Vec<_> = (0..3)
        .map(|i| {
            service
                .create_session(&format!("stress-session-{}", i))
                .unwrap()
        })
        .collect();
    ...
    assert_eq!(
        metrics.total_checkpoints, 90,
        "Should have 90 checkpoints total"
    );
    ...
    assert!(
        health.healthy,
        "System should be healthy after stress test: {}",
        health.message
    );
}
