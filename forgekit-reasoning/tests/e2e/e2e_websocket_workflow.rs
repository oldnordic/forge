...
async fn e2e_websocket_client_workflow() {
    ...
    client
        .send(Message::Text(serde_json::to_string(&create_cmd).unwrap()))
        .await
        .unwrap();
...
async fn e2e_websocket_multi_client() {
    ...
    let msg = Message::Text(serde_json::to_string(&subscribe_cmd).unwrap());
    ...
    let event1 = timeout(Duration::from_secs(5), client1.next())
        .await
        .unwrap()
        .unwrap()
        .unwrap();
    let event2 = timeout(Duration::from_secs(5), client2.next())
        .await
        .unwrap()
        .unwrap()
        .unwrap();
    ...
    assert_eq!(
        event1.data.get("checkpoint_id"),
        event2.data.get("checkpoint_id")
    );
...
async fn e2e_websocket_authentication() {
    ...
    let mut server = CheckpointWebSocketServer::with_config("127.0.0.1:0", service.clone(), config);
    let addr = server.start().await.unwrap();
    ...
    client
        .send(Message::Text(serde_json::to_string(&auth_cmd).unwrap()))
        .await
        .unwrap();
