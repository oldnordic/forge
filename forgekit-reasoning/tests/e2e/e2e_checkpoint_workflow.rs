...
fn e2e_complete_debugging_session() {
    ...
    let cp1 = service
        .checkpoint(&session, "Initial state - tensor dims look wrong")
        .unwrap();
...
fn e2e_multi_session_debugging() {
    ...
    // Back to issue #1
    let _ = service
        .checkpoint(&session1, "Issue 1 - resolution")
        .unwrap();
...
fn e2e_compaction_workflow() {
    ...
    assert!(
        after.len() <= 10,
        "Should have fewer checkpoints after compaction"
    );
}
