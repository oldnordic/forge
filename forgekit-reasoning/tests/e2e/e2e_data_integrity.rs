...
fn e2e_health_check_with_validation() {
    ...
    assert!(
        health.message.contains("validated"),
...
fn e2e_export_import_with_integrity() {
    ...
    let _ = service
        .execute(CheckpointCommand::Create {
            session_id: session,
    ...
    assert!(
        export_data.contains("checksum"),
        "Export should contain checksums"
    );
    assert!(
        export_data.contains("Before fix"),
        "Export should contain checkpoint data"
    );
    assert!(
        export_data.contains("After fix"),
        "Export should contain checkpoint data"
    );
    ...
    assert_eq!(
        new_service.global_sequence(),
        2,
        "Global sequence should be restored"
    );
}
    ...
    for thread_id in 0..4 {
        ...
        let handle = thread::spawn(move || {
            for i in 0..10 {
                ...
                assert!(
                    svc.validate_checkpoint(&id).unwrap(),
