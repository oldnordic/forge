...
async fn test_websocket_client_connect() {
    ...
    let msg = tokio_tungstenite::tungstenite::Message::Text(serde_json::to_string(&cmd).unwrap());
...
async fn test_websocket_event_broadcast() {
    ...
    let _ = timeout(Duration::from_secs(2), client1.next())
        .await
        .unwrap();
    let _ = timeout(Duration::from_secs(2), client2.next())
        .await
        .unwrap();
    ...
    let event1 = timeout(Duration::from_secs(2), client1.next())
        .await
        .unwrap()
        .unwrap()
        .unwrap();
    let event2 = timeout(Duration::from_secs(2), client2.next())
        .await
        .unwrap()
        .unwrap()
        .unwrap();
...
async fn test_websocket_malformed_message() {
    ...
    // Send malformed JSON
    let msg = tokio_tungstenite::tungstenite::Message::Text("this is not valid json".to_string());
    client.send(msg).await.unwrap();
...
async fn test_websocket_multiple_sessions() {
    ...
    let msg = tokio_tungstenite::tungstenite::Message::Text(serde_json::to_string(&cmd1).unwrap());
    client.send(msg).await.unwrap();
    ...
    let msg = tokio_tungstenite::tungstenite::Message::Text(serde_json::to_string(&cmd2).unwrap());
    client.send(msg).await.unwrap();
...
async fn test_websocket_client_disconnect() {
    ...
    let response = timeout(Duration::from_secs(2), client2.next())
        .await
        .unwrap()
        .unwrap()
        .unwrap();
    ...
    assert!(
        response.success,
        "Server should work after client disconnect"
    );
...
async fn test_websocket_authentication() {
    ...
    let msg =
        tokio_tungstenite::tungstenite::Message::Text(serde_json::to_string(&auth_cmd).unwrap());
    client.send(msg).await.unwrap();
...
async fn test_websocket_high_volume() {
    ...
    let msg = tokio_tungstenite::tungstenite::Message::Text(serde_json::to_string(&cmd).unwrap());
    client.send(msg).await.unwrap();
    let _ = timeout(Duration::from_secs(2), client.next())
        .await
        .unwrap();
    ...
    for i in 0..50 {
        ...
        let msg =
            tokio_tungstenite::tungstenite::Message::Text(serde_json::to_string(&cmd).unwrap());
        client.send(msg).await.unwrap();
    }
    ...
    assert!(
        responses >= 50,
        "Should receive all responses, got {}",
        responses
    );
