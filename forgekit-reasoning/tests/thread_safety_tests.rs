...
fn test_concurrent_checkpoint_creation() {
    ...
    let handles: Vec<_> = (0..10)
        .map(|i| {
            ...
            thread::spawn(move || {
                ...
                for j in 0..5 {
                    manager
                        .checkpoint(format!("Thread {} - Checkpoint {}", i, j))
                        .unwrap();
                }
            })
        })
    ...
    assert_eq!(
        checkpoints.len(),
        50,
        "All concurrent checkpoints should be created"
    );
}
...
fn test_concurrent_read_write() {
    ...
    assert!(
        checkpoints.len() >= 10,
        "Should have at least initial checkpoints"
    );
}
        ...
        .map(|_| {
            ...
            thread::spawn(move || {
                ...
                assert_eq!(
                    unique.len(),
                    local_sequences.len(),
                    "Local sequences should be unique"
                );
            })
        })
        .collect();
...
fn test_concurrent_compaction() {
    ...
    assert!(
        ...
        "Should have some checkpoints after concurrent ops"
    );
...
fn test_thread_safe_session_isolation() {
    ...
    assert!(
        ...
        "Sessions should not share checkpoints"
    );
}
...
fn test_stress_high_concurrency() {
            ...
            thread::spawn(move || {
                ...
                for op_id in 0..ops_per_thread {
                    ...
                    match op_id % 4 {
                        0 => {
                            manager
                                .checkpoint(format!("T{}-C{}", thread_id, op_id))
                                .unwrap();
                        }
                        1 => {
                            let _ = manager.list();
                        ...
                        2 => {
                            let _ = manager.list_by_session(&session_id);
                        }
                        3 => {
                            let _ = manager.get(&CheckpointId::new());
                        } // May not exist
                        _ => unreachable!(),
                    }
                }
    ...
    assert!(
        !checkpoints.is_empty(),
        "Should have checkpoints after stress test"
    );
