...
fn test_checkpoint_includes_checksum() {
    ...
    assert!(
        !cp.checkpoint.checksum.is_empty(),
        "Checkpoint should have checksum"
    );
}
...
fn test_service_validate_checkpoint() {
    ...
    assert!(
        result.unwrap(),
        "Service validation should return true for valid checkpoint"
    );
}
...
fn test_export_includes_checksums() {
    ...
    assert!(
        export.contains("checksum"),
        "Export should include checksums"
    );
}
...
fn test_health_check_validates_checkpoints() {
    ...
    assert!(
        health.healthy,
        "Health check should pass with valid checkpoints"
    );
}
...
fn test_concurrent_validation() {
    ...
    for _ in 0..4 {
        ...
        let handle = thread::spawn(move || svc.validate_all_checkpoints().unwrap());
        handles.push(handle);
    }
...
fn test_validation_detects_tampering() {
    ...
    assert!(
        result.is_err(),
        "Validation should fail for tampered checkpoint"
    );
}
