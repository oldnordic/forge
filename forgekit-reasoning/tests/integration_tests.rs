...
fn test_service_multi_session() {
    let service = Arc::new(CheckpointService::new(
        ThreadSafeStorage::in_memory().unwrap(),
    ));
    ...
    service
        .checkpoint(&session1, "Session 1 checkpoint")
        .unwrap();
    service
        .checkpoint(&session2, "Session 2 checkpoint")
        .unwrap();
...
fn test_service_auto_checkpoint() {
    ...
    service
        .trigger_auto_checkpoint(&session, AutoTrigger::VerificationComplete)
        .unwrap();
...
async fn test_checkpoint_streaming() {
    ...
    assert!(
        event.is_ok(),
        "Should receive checkpoint event within timeout"
    );
...
fn test_service_api_commands() {
    ...
    let list_cmd = CheckpointCommand::List {
        session_id: session,
    };
    ...
    for i in 0..5 {
        ...
        let handle = std::thread::spawn(move || {
            let session = service_clone
                .create_session(&format!("concurrent-{}", i))
                .unwrap();
            for j in 0..5 {
                service_clone
                    .checkpoint(&session, format!("CP-{}", j))
                    .unwrap();
            }
