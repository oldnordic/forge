...
pub struct Verifier {
    ...
    forge: Option<forgekit_core::Forge>,
...
impl Verifier {
    ...
    pub fn with_forge(forge: forgekit_core::Forge) -> Self {
        Self {
            forge: Some(forge),
            llm: None,
        }
                ...
                return Ok(output
                    ...
                    .filter_map(|d| {
                        let level = match d.severity {
                            forgekit_core::diagnostic::DiagnosticSeverity::Error => {
                                Some(DiagnosticLevel::Error)
                            }
                            forgekit_core::diagnostic::DiagnosticSeverity::Warning => {
...
mod tests {
    ...
    async fn test_verifier_with_forge_uses_build_module() {
        let temp_dir = tempfile::tempdir().unwrap();
        let forge = forgekit_core::ForgeBuilder::new()
            .path(temp_dir.path())
