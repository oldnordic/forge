...
impl AgentLoop {
    ...
    pub(super) async fn verify_phase(
        ...
        if let Some(hyp_id) = self.current_hypothesis {
            let runner = forgekit_reasoning::VerificationRunner::new(
                Arc::new(self.reasoning.board.clone()),
                1,
            );
            ...
            let (on_pass, on_fail) = if report.passed {
                (
                    Some(
                        forgekit_reasoning::verification::check::PassAction::SetStatus(
                ...
                (
                    None,
                    Some(
                        forgekit_reasoning::verification::check::FailAction::SetStatus(
