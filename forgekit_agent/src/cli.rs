...
//!
//! ```bash
//! $ forgekit-agent run "Add authentication to all API endpoints"
//! ```
//!
//! Plan only (dry run):
//!
                ...
                match agent.run_with_runtime(&mut runtime, &query).await {
                    Ok(_) => {
                        ...
                        if verbose {
                            ...
                            println!(
                                "    Cache hits: {}",
                                runtime.metrics().count(forgekit_runtime::MetricKind::CacheHit)
                            );
                            println!(
                                "    Cache misses: {}",
                                runtime
                                    .metrics()
                                    .count(forgekit_runtime::MetricKind::CacheMiss)
                            );
...
mod tests {
    ...
    async fn test_cli_accepts_with_runtime_flag() {
        ...
        let cli = Cli::try_parse_from(["forgekit-agent", "run", "test query", "--with-runtime"]);
        assert!(cli.is_ok());
