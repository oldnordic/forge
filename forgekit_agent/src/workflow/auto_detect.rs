...
use forgekit_core::graph::GraphModule;
    ...
    pub async fn detect_dependencies(
        ...
        for node_idx in workflow.graph.node_indices() {
            if let Some(node) = workflow.graph.node_weight(node_idx) {
                ...
                // TaskNode stores the task as a trait object, so we cannot
                // downcast to GraphQueryTask here. We extract the target from
                // the task name instead. A future TaskNode metadata API could
                // replace this heuristic with direct field access.
