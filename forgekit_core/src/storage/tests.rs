...
#[test]
fn test_lookup_registry_finds_forgekit_core() {
    let src_dir = std::path::Path::new("/home/feanor/Projects/forge/forgekit_core/src");
    if !src_dir.exists() {
        return;
    }
    ...
    let db_str = db.to_string_lossy();
    assert!(
        db_str.contains("forge/forgekit-core.db"),
        "expected registry match for forgekit_core/src, got: {}",
        db_str
    );
}

#[test]
fn test_lookup_registry_finds_forgekit_agent() {
    let src_dir = std::path::Path::new("/home/feanor/Projects/forge/forgekit_agent/src");
    if !src_dir.exists() {
        return;
    }
    let db = default_db_path(src_dir);
    let db_str = db.to_string_lossy();
    assert!(
        db_str.contains("forge/forgekit-agent.db"),
        "expected registry match for forgekit_agent/src, got: {}",
        db_str
    );
}
...
fn test_lookup_registry_from_codebase_path_without_src() {
    let crate_dir = std::path::Path::new("/home/feanor/Projects/forge/forgekit_core");
    if !crate_dir.exists() {
        return;
    }
    ...
    assert!(
        db_str.contains("forge/forgekit-core.db"),
        "expected registry match for forgekit_core (without /src), got: {}",
        db_str
    );
}
...
fn test_lookup_registry_from_agent_codebase_path() {
    let crate_dir = std::path::Path::new("/home/feanor/Projects/forge/forgekit_agent");
    if !crate_dir.exists() {
        return;
    }
    let db = default_db_path(crate_dir);
    ...
    assert!(
        db_str.contains("forge/forgekit-agent.db"),
        "expected registry match for forgekit_agent (without /src), got: {}",
        db_str
    );
}
