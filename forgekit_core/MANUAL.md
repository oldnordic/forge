...
**If the Agent claims a function does X, but the forgekit_core graph shows the CFG never reaches that path, the Contradiction Detector flags it.**
...
```rust
use forgekit_core::Forge;
use forgekit_reasoning::{HypothesisBoard, Evidence, Confidence};
...
let ws = forgekit_reasoning::WebSocketServer::new(8080);
...
```bash
cargo add forgekit_core --features treesitter-cfg
```
