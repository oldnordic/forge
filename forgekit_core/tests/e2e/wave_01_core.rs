...
async fn e2e_forge_initialization_default() {
    let (_temp_dir, project_path) = super::unique_project_dir();
...
async fn e2e_forge_creates_directory_structure() {
    ...
    // DB lives at the path Forge resolves (registry or ~/.magellan/<stem>/<stem>.db)
    assert!(
        forge.db_path().exists(),
        "database file should be created at {}",
        forge.db_path().display()
    );
}
