...
4. test_symbol_id_zero - Verifies SymbolId(0) is valid and displays correctly
...
3. test_block_id_zero - Verifies BlockId(0) is valid
...
4. test_path_id_empty - Verifies PathId from empty bytes produces valid ID
...
4. test_location_clone - Verify Location derives Clone correctly
5. test_location_zero_length - Verify Location with byte_start == byte_end is valid
...
6. test_span_merge_adjacent - Verifies merging adjacent spans (end of first == start of second)
7. test_span_overlaps - Verifies overlaps() method correctly detects overlapping spans
...
3. test_symbol_kind_is_variable - Verifies Local/Static/Param return true for variable-like kinds
...
3. test_language_from_str - Verifies Language can be parsed from string representations

**PathKind Tests (2 tests):**
1. test_path_kind_variants - All four variants can be constructed
2. test_path_kind_is_absolute - Verifies is_absolute() method works correctly

**ReferenceKind Tests (2 tests):**
1. test_reference_kind_variants - All eight variants can be constructed
2. test_reference_kind_is_call - Verifies Call/MethodCall return true for call-like references
...
3. test_symbol_with_parent - Verify Symbol with parent_id works
4. test_reference_with_metadata - Verify Reference with metadata JSON works

**Edge Case Tests (3 tests):**
1. test_large_symbol_id - Verifies SymbolId with large value (i64::MAX) works
2. test_large_byte_offsets - Verifies Location with large byte offsets (u32::MAX) works
3. test_empty_metadata - Verifies Symbol/Reference with empty metadata JSON works

Total: 40 tests for types.rs

Use tokio::test for async tests, plain #[test] for sync tests.
  </action>
  <verify>
Run: cargo test -p forge_core types
Expected: ~40 new tests pass, all assertions for type behavior pass
  </verify>
  <done>
forge_core/src/types.rs has #[cfg(test)] mod tests with 40 test functions covering: ID types (Display, From, Ord, edge cases), Location/Span methods (including merge, overlaps), enum variants with predicates, data structure construction with metadata, and large value edge cases. All tests pass.
  </done>
</task>
